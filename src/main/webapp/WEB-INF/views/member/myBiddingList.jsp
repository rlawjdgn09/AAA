...
<section id="projects" class="padding-top">
	...
	<div class="row">
			...
			<div class="pull-right">
           		<form id='searchForm' action="${contextPath}/cmBoard/cmBoardGetList" method="get">
					...
					<select class="form-control" name="searchType">
						<option value="TCW" ${pageMaker.cri.searchType eq 'TCW' or null and not empty pageMaker.cri.searchKeyword ? "selected" : ""}>전체(검색목록)</option>
   	...
   	<ul id="tab1" class="nav nav-tabs">
     	<li class="active" id="myBiddingTab" ><a href="#tab1-item1" data-toggle="tab" aria-expanded="false">입찰중 내역</a></li>
        <li class="" id="myWinningTab" ><a href="#tab1-item2" data-toggle="tab" aria-expanded="true">낙찰 내역</a></li>
   	</ul>
...
<script src="${contextPath}/resources/closer/myBiddingBoard.js"></script>
<script>
//csrf 공격 방지
...
showList(pageNum, amount, category, searchType, searchKeyword, memberId, flag);

$("#myBiddingTab").on("click", function() {
	flag = "F";
	var board = {pageNum : pageNum, amount:amount, category:category, searchType:searchType,
					 searchKeyword:searchKeyword, memberId:memberId, flag:flag}
...
$("#myWinningTab").on("click", function() {
	flag = "T";
	var board = {pageNum : pageNum, amount:amount, category:category, searchType:searchType,
				 searchKeyword:searchKeyword, memberId:memberId, flag:flag}
	memBoardService.getTotalPageCnt(
...
//리스트 보여주기
function showList(pageNum, amount, category, searchType, searchKeyword, memberId, flag){
	memBoardService.getList(
		...
		function(list){
			...
			if(list==null || list.length==0){
				str +="<tr stlye='text-align:center;'>"
			...
			for(var i=0, len=list.length || 0; i<len; i++){
				var myPrice = list[i].myPrice

				str +="<tr>"
					+ "		<td>"+list[i].bno+"</td>"
					//+ "		<td>"+list[i].category+"</td>"
					+ "		<td><a class='move' href='"+list[i].bno+"'>"+list[i].title+"</a></td>"
					+ "		<td>"+ myPrice.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") +"</td>"
					+ "		<td>"+memBoardService.showTime(list[i].biddingDate)+"</td>"
					+ "</tr>";
			}//for-end
			tableListBody.html(str);
...
$("#searchForm button").on("click",function(e){
	...
	pageNum = 1;
		...
		var board = {pageNum : pageNum, amount:amount, category:category,
					 searchType:searchType, searchKeyword:searchKeyword, memberId:memberId, flag:flag}
   		memBoardService.getTotalPageCnt(
			board,
			function(result){
				totalPageCnt = result;
